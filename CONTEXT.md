...
- The favicon/app mark is four filled `nX=3`, `nY=3` Lamé superellipses in the base action blue (`#0B5FFF`) on transparency. The four-tile group is uniformly scaled from its original `7×7` tile geometry so its outside edges span the complete `24×24` viewBox formerly occupied by the enclosing tile, preserving the original tile-to-gap proportions. The curves follow Squircle Portal's 64-segment sampling and two-decimal coordinate precision. Favicon URLs carry a revision query when the artwork changes to avoid stale browser caches. The page header reuses the same path, full-viewBox group transform, and transparent composition, filled with the theme's current action blue (`--accent`).
