...
- The collection selector uses Keel's `keel-select keel-select--sm` primitive. Frontend asset revisions and the `fluent-icon-browser-v14` app-shell cache advance together so installed clients receive selector and preview updates.
...
- AWS Architecture Icons remain a candidate/blocker: the current official architecture ZIP is CORS-readable and SVG-only, but its visible terms grant diagram creation without establishing public indexing, runtime retrieval, or copy/download rights. The browser currently supports npm tgz entry extraction, not ZIP extraction, so AWS is not published.
- Google Cloud remains a blocked candidate: the official category/core ZIPs contain SVGs but are not CORS-readable, expose no per-icon SVG path, and their licence/brand terms are insufficient for this public runtime and copy/download contract. It is not published.
