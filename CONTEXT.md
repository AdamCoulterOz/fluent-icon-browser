...
- Salesforce SLDS `standard`, `action`, `doctype`, `custom`, and `utility` icons from the official `@salesforce-ux/icons` archive, including first-party MuleSoft artwork
- AWS Architecture Icons from the official Architecture Icons archive
- Google Cloud Console icons discovered from its public route map and referenced public gstatic MicroUI/StandaloneUI modules
...
- icon-set switching (Fluent/Segoe/Azure/Flight/HashiCorp/Salesforce/AWS/Google Cloud/Red Hat)
...
- `script.js`: browser logic for loading/filtering/rendering icon data and modal actions. Search indexes names, descriptions, aliases, visible metaphors, and hidden search terms; the details panel renders visible metaphors only.
- `process.py`: legacy/optional icon transform script (kept for reference, not used in CI pipeline).
- `generate-icon-data.py`: builds `icon-data.json` directly from upstream `assets` plus ordinary/branded MDL2 components and emits commit-pinned source URLs + native sizes. It separates concise human-readable `metaphors` for visible chips from non-display source metadata in `searchTerms`; both remain searchable.
- `flight_icons.py`, `redhat_icons.py`, `salesforce_icons.py`, and `aws_icons.py`: validate approved source subsets, write/read digest-bound locks, and emit remote SVG references without committing payloads. Salesforce uses a registry archive lock with approved entry digests; AWS uses its official Architecture Icons ZIP and approved entry digests; Flight's generic concepts and HashiCorp product marks have independent locks.
- `gcp_console_icons.py`: derives public gstatic MicroUI/StandaloneUI modules from the public Console route map, validates and retains literal SVG templates only, and writes the deterministic `gcp-console-icons/` source tree with manifest, source lock, and required attribution/no-endorsement notice. It never persists source JavaScript.
- `source_lock.py`: source-file/path and package-archive digest helpers shared by digest-bound collection locks.
- `azure_portal_icons.py`: discovers the bounded public Azure Portal source, validates count/drift gates, and emits descriptor-only Azure records plus a compact source lock; it never retains SVG payloads. A source must contain renderable vector artwork: SVG wrappers that contain raster `<image>` or `foreignObject` content, including local `<use>` references to embedded raster data, are omitted.
- `remote-icon-source.js`: resolves Azure descriptors lazily, Salesforce/AWS ZIP entries, and same-origin GCP archive entries after archive/entry SHA-256 verification, then sanitizes the SVG before it is previewed, copied, or downloaded. `previewThemeColor` changes only the rendered preview: inline monochrome SVG paint becomes theme `currentColor`; owner-hosted monochrome SVG images remain black in light mode and invert white in dark mode. Source bytes used for runtime resolution, copy, and download remain unchanged.
...
- `.upstream-flight-icons.json`, `.upstream-hashicorp-products.json`, `.upstream-aws-icons.json`, `.upstream-redhat-icons.json`: committed package/archive version, pinned commit or release, approved subset, and source-content digest locks.
- `gcp-console-icons/`: committed SVG-only Google Cloud Console source tree with `source-lock.json`, `manifest.json`, and `REFERENTIAL-FAIR-USE.md`; Pages packages it as `gcp-console-icons.zip`, which is never committed.
...
- `.github/workflows/sync-fluent-icons.yml`
  ...
  - checks approved upstream revisions and source-lock drift for:
    ...
    - Salesforce SLDS official `@salesforce-ux/icons` archive (digest-bound `standard`, `action`, `doctype`, `custom`, and `utility` entries; five genuine individual-SVG families total 1,780; `*-sprite` SVG/RTL support sheets excluded)
    - AWS Architecture Icons official ZIP (archive and selected-entry digests)
    - Google Cloud Console public route map and referenced public gstatic MicroUI/StandaloneUI modules (literal SVG templates only; source tree/lock/notice validated before archive packaging)
  ...
  - generates combined index from:
    ...
    - official Salesforce registry archive entries and AWS Architecture Icons archive entries verified against archive and per-entry digest locks before browser extraction
    - commit-pinned Red Hat `standard`/`ui`/`microns` SVG references verified against a digest-bound source lock
  - generates AWS Architecture descriptors from the official archive while retaining only entry metadata and digests in the committed index/lock
  - commits updated index/metadata and source-lock records; no Azure, Flight, HashiCorp Products, Salesforce, AWS, or Red Hat SVG payload. The sole exception is the validated Google Cloud Console SVG-only source tree; Pages packages it into the runtime ZIP.
...
- `keel.css` is vendored from the official Keel tag `v0.4.4` by `update-keel.sh`; its asset revision and cache advance with bundle updates.
...
- Fluent and Segoe canonical families each carry one generated deterministic semantic category from the shared UI-domain taxonomy: Accessibility; Security & Privacy; Status & Feedback; People & Identity; Communication & Collaboration; Files & Documents; Media; Devices & Hardware; Data & Developer; Commerce & Finance; Places & Travel; Nature & Animals; Layout & Formatting; Shapes & Symbols; Actions & Navigation; or General UI. Classification uses canonical name tokens and explicit precedence only; descriptions, metaphors, URLs, and search terms do not decide it. Segoe additionally has Microsoft Product Marks, requiring an approved Microsoft identity plus `logo`/`wordmark` or explicit Viva Engage, and Microsoft Product UI, requiring branded source provenance plus an approved Microsoft product identity. Branded metaphors are preserved; private provenance is not published.
...
- Published set keys are `fluent`, `segoe`, `azure`, `flight`, `hashicorp`, `salesforce`, `aws`, `gcp`, and `redhat`; `fabric` remains a compatibility alias to `segoe` only while no direct `fabric` set exists. Generated counts are observations rather than permanent contracts. AWS is the canonical `aws` set and currently contains 809 families across 45 generated source categories.
- Collections with more than one source-supplied `icon.category` show a Keel-native group selector. The group selector combines with search and style filtering, resets on collection switch, and does not add a group parameter to the stable `?set=<key>&icon=<name>` URL. Collections without a multi-category source taxonomy do not show the selector.
- Flight package 5.1.0 is MPL-2.0 and includes generic concepts only: `Products` and `Services` are excluded, and its existing `-fill` grouping is unchanged. The separately locked `hashicorp` Products set combines upstream `<base>`, `<base>-fill`, and `<base>-color` SVGs under canonical `?set=hashicorp&icon=<base>` links as regular, filled, and color variants; named fill/color source forms remain searchable and deep-link to that canonical family rather than becoming separate links. Only pure-black `color` source previews carry `previewThemeColor`; multi-colour artwork remains source-coloured. Red Hat package 2.3.1 is CC BY 4.0 and includes only `standard`, `ui`, and `microns`; `social` is excluded. Both use commit-pinned upstream SVG URLs and digest-bound locks; no new SVG payloads are committed.
- Salesforce SLDS package 10.17.0 is CC BY-ND 4.0 and includes source-colour `standard`, `action`, `doctype`, `custom`, and `utility` artwork, including MuleSoft; these five genuine individual-SVG families total 1,780. The five `*-sprite` SVG/RTL sprite sheets remain excluded as generated support artifacts, and `Product` is not a sixth `@salesforce-ux/icons` category. Its official registry archive and each selected entry are SHA-256 locked; the client verifies both before extraction/sanitization, and source-transforming output controls are unavailable. Light-only Salesforce artwork carries `previewThemeColor` and renders as a black foreground in light mode or white in dark mode, without a preview background.
- The collection selector uses Keel's `keel-select keel-select--sm` primitive. Frontend asset revisions and the `fluent-icon-browser-v19` app-shell cache advance together so installed clients receive selector, archive extraction, and preview updates.
- `SOURCES.md` is the concise source-eligibility registry. It distinguishes published collections from candidates requiring a deterministic official source boundary (revisioned/stable per-icon URLs or an official archive plus entry descriptor/digest), unauthenticated CORS client access when runtime retrieval remains owner-hosted, compatible terms for automated indexing/deep-linking/hotlinking/runtime retrieval/user copy-download, and attribution/trademark/no-endorsement treatment. No rehosting is a separate implementation invariant: sync may temporarily inspect official payloads, then retains only generated index/lock metadata while the client resolves icons from the source owner at runtime. The Google Cloud Console SVG tree is the documented static-archive exception. It does not provide legal advice. A local-only licensed-pack architecture is not implemented.
...
- Azure colour filtering is capability-based: a regular, filled, or color variant is colour-capable when its descriptor has `preserveSourceColors`. The current generated observation is 156 colour-capable Azure families out of 1,372; these counts are not permanent contracts. Upstream variant keys and public deep links remain unchanged.
- Azure discovery is bounded to the deterministic default public surface from portal bootstrap, current RequireConfig/dependency data, and default extension-manifest hashes; schema drift or count collapse fails the sync. The accepted all-1,372-family review assigns every card one deterministic browse category: `General UI` is distinct from Portal UI/surface groups such as `Portal Commands` and `Browse & Discover`; typed resource icons use ARM-provider namespaces; exact reviewed core source names map into the same flat domain labels, such as `Identity`, `Compute`, `Networking`, `Shared`, and observed `DevOps`; and genuine unknown/third-party resources remain `Other Providers`. Categories are browse metadata only: they never change public identifiers, variants, source URLs, deep links, source bounds, or the 105 local hold. Generated counts remain observations, not contracts. The 105 local legacy Documents SVGs remain read-only, unimported, and unpublished: none has a canonical content-hash match in the generated source; reconciliation found 23 exact-name metadata counterparts, 28 high-confidence renamed counterparts, 43 ambiguous items, and 11 initially absent metadata candidates.
...
- AWS Architecture Icons are published under canonical set key `aws` for technical documentation and architecture diagrams per the product decision. The accepted bounded source is the official AWS Architecture Icons ZIP: the generated source lock records the release and archive digest, each selected entry has its own digest, and the browser verifies both before client-side ZIP extraction and SVG sanitization. The index retains descriptors and metadata only; no upstream SVG payload is committed or rehosted. Generated `sourceCategory` values feed the existing group selector. AWS `Light`/`Dark` exports merge into one canonical family only when they are exact terminal-name pairs at every matching size; a generic per-size `themeSources` descriptor selects the matching source for `prefers-color-scheme`, while former themed names remain aliases. AWS output does not expose source-transforming controls.
- Google Cloud Console is published as canonical `gcp`. Its deterministic public discovery begins at `https://console.cloud.google.com/p/routemapdata`, which names every public GCP MicroUI/StandaloneUI module currently on that route-map surface. The current accepted generation is 257 modules and 4,363 literal SVG templates; 535 lack `data-icon-name` and retain a deterministic opaque metadata fallback. The browse catalogue exposes 250 cards: 106 `Resource Icons` families and 144 `Common UI` families. Resource identity removes terminal `SectionIcon`/`Icon` component suffixes, normalizes established GCP product spellings, and retains authored size/style choices under one card when no style-and-size source collision exists. `regular` means stroke-led outlined geometry, `filled` means monochrome filled geometry, and `color` means intentional authored multicolour paint; source SVG bytes remain unchanged. Shared resource/context names are classified independently of their source module. Cross-module exact digest matches become `Common UI`; unclassified module-local templates stay in the complete manifest/source tree but are deliberately omitted from browsing. Source-authored empty templates remain manifest evidence but are omitted from the browse catalogue; safe local SVG `<use>` fragments are retained by runtime sanitization. Live Console inspection also observed a separate 248-icon core resource library in versioned `OneCloudBarMicroUi`/`CloudConsoleWeb` chunks, including `databasesSectionIcon` and `kmsSectionIcon`; those public assets are not advertised by `routemapdata`, and their current URLs require an authenticated Console bootstrap to discover. Do not treat the route-map corpus as complete Console coverage or manufacture a core URL from a stale token. The source tree includes the supplied `REFERENTIAL-FAIR-USE.md` trademark, functional-diagram, independence, and no-endorsement notice. Pages creates the same-origin `gcp-console-icons.zip` deploy artifact; it is not versioned. This is a bounded exception, not a general Google proxy or a licence conclusion.
