...
- `.github/workflows/sync-fluent-icons.yml`
  ...
  - commits updated index/metadata and source-lock records; no Azure, Flight, HashiCorp Products, Salesforce, AWS, or Red Hat SVG payload
...
- Published set keys are `fluent`, `segoe`, `azure`, `flight`, `hashicorp`, `salesforce`, `aws`, and `redhat`; `fabric` remains a compatibility alias to `segoe` only while no direct `fabric` set exists. Generated counts are observations rather than permanent contracts. AWS is the canonical `aws` set and currently contains 859 families across 45 generated source categories.
...
- The collection selector uses Keel's `keel-select keel-select--sm` primitive. Frontend asset revisions and the `fluent-icon-browser-v15` app-shell cache advance together so installed clients receive selector, archive extraction, and preview updates.
...
- Google Cloud remains unindexed. The current owner archives are the `services.google.com` category/core ZIP files from the current 2025 assets; they provide no ACAO and their preflight responds 405. No qualifying Google-owned CORS mirror was found. Build-time indexing alone cannot make browser extraction work, and no proxy or rehosting is implemented.
