...
- Published set keys are `fluent`, `segoe`, `azure`, `flight`, `hashicorp`, `salesforce`, `aws`, `gcp`, and `redhat`; `fabric` remains a compatibility alias to `segoe` only while no direct `fabric` set exists. Generated counts are observations rather than permanent contracts. AWS is the canonical `aws` set and currently contains 859 families across 45 generated source categories.
