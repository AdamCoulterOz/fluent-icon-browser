...
- The only published set keys are `fluent` and `segoe`; `fabric` remains a generated compatibility alias for existing deep links. Alias resolution applies only when no direct set matches, so a future real `fabric` collection would win and requires an explicit compatibility decision. The collection model permits future coherent sets, but it does not authorize importing legacy Azure assets; Azure provenance and licensing remain unresolved, and the Azure Architecture Center terms are limited to architecture diagrams, training materials, or documentation, so no Azure source, URL, asset, or key belongs in this repository without explicit permission or clearer source-package terms.
