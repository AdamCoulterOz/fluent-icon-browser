...
- Azure is limited to the deterministic default public Portal core and extension-manifest surface. Its source SVGs are resolved lazily from public sources; the repository and Pages artifact contain no Azure SVG payloads. A descriptor is published only when its source contains renderable vector artwork; SVG wrappers containing raster `<image>` or `foreignObject` content, including local `<use>` references to embedded raster data, are excluded. Azure cards carry one deterministic browse category: `General UI`, provider-derived `Resource / <domain>`, or an explicit Portal surface group. Provider categories use ARM namespaces only; icon labels and descriptions do not affect grouping. Colour filtering is capability-based: any regular, filled, or color variant whose descriptor has `preserveSourceColors` is colour-capable. The current generated observation is 156 colour-capable families out of 1,372; counts are not permanent contracts, and upstream variant keys and public deep links remain unchanged.
...
- Deep-link query parameters are the public contract; DOM shape, internal adapters, normalizers, and workflow implementation are not. Visible `metaphors` are concise browse chips; implementation/source metadata and prose are retained as non-displayed `searchTerms`, so search remains inclusive without exposing transport noise.
