...
- Google Cloud Console Icons is published as canonical set `gcp`. Sync derives the complete public route map from `https://console.cloud.google.com/p/routemapdata`, fetches the referenced public `www.gstatic.com` MicroUI/StandaloneUI modules, and retains only literal SVG templates. The current observation is 4,363 templates from 257 modules and 418 renderable browse cards; 535 source templates have no `data-icon-name` and retain a deterministic opaque metadata fallback. The catalogue folds exact retained SVG digests shared across module IDs into `Common UI`; entries confined to one module remain module-specific even when their names match, and replaced generated names remain aliases for `?set=gcp&icon=` links. Source-authored non-renderable templates stay in the source tree/manifest but are excluded from the catalogue. Runtime sanitization retains only local-fragment `<use>` references so valid source templates remain previewable without admitting external SVG references. The resulting deterministic `gcp-console-icons/` source tree contains SVGs, a manifest, a source lock, and the required `REFERENTIAL-FAIR-USE.md` attribution/no-endorsement notice; no source JavaScript is retained. Pages deterministically packages that tree as same-origin `gcp-console-icons.zip`, whose archive and entry digests are verified before extraction and sanitization.
